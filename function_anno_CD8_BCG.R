...
dif_BCG_CON <- Dif_cluster_enrich(seurat_object = CD8_T_cell,clustera = "870_BCG",clusterb = "CON",group.by = NULL, out_dir = "/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON", OrgDb = "org.Mm.eg.db", sig_gene_p = 0.05, sig_gene_fd = 1, organism = "mmu", cut_p = "p_val")
...
ggsave(p, file="/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON/up_bubble_plot.pdf", width=6, height=4)


dif_BCG_CON.GO_down <- select(dif_BCG_CON$GO_down, c("ONTOLOGY","ID","Description","pvalue","Count","GeneRatio"))
dif_BCG_CON.GO_down[,1] <- paste0("GO.",dif_BCG_CON.GO_down[,1])

dif_BCG_CON.GO_down <- split(dif_BCG_CON.GO_down, f=dif_BCG_CON.GO_down$ONTOLOGY)
dif_BCG_CON.GO_down <- lapply(dif_BCG_CON.GO_down, function(x){return(x[1:5,])})
dif_BCG_CON.GO_down <- data.frame(Reduce(rbind, dif_BCG_CON.GO_down))
#KEGG down
...
dif_BCG_CON.GO_down <- rbind(dif_BCG_CON.GO_down, dif_BCG_CON.KEGG_down)
dif_BCG_CON.GO_down$GeneRatio <- GeneRatio(dif_BCG_CON.GO_down$GeneRatio)


dif_BCG_CON.GO_down$Count <- as.character(dif_BCG_CON.GO_down$Count)
p <- ggplot(dif_BCG_CON.GO_down, aes(x=Description,y=Count))+geom_point(aes(color=-log10(pvalue),size=GeneRatio))+
coord_flip()+scale_color_gradientn(colors=c("#5D2265","#0095A2","#FCE557"))+theme_bw()+
facet_grid(ONTOLOGY ~ ., scales = "free", space='free') 
ggsave(p, file="/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON/down_bubble_plot.pdf", width=6, height=4)


library(openxlsx)
dif_BCG_CON_gene_up <- read.xlsx("/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON/dif_gene.xlsx", sheet=1)
dif_BCG_CON_gene_down <- read.xlsx("/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON/dif_gene.xlsx", sheet=2)
...
ggsave(p, file="/hengya/home/lixin/Project/870_Mouse/5_T_cell/function_anno/CD8/BCG_CON/heatmap.pdf", width=4, height=3)
